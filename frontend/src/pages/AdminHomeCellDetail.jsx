...
export default function AdminHomeCellDetail() {
  const { id } = useParams()
  ...
  const load = () => {
    ...
    Promise.all([
      ...
      .then(([homecellRes, membersRes, usersRes, assignmentsRes]) => {
        setForm(homecellRes.data)
        setMembers(membersRes.data || [])
        setAllUsers(usersRes.data?.results || [])
        setAssignments(assignmentsRes.data || [])
      })
      .catch((err) => setError(readableError(err, 'Unable to load this Home Cell Fellowship.')))
  ...
  const saveForm = async (e) => {
    ...
    try {
      ...
      const res = await api.patch(`/admin/homecells/${id}/`, payload)
      setForm(res.data)
      setNotice('Saved.')
    } catch (err) {
  ...
  const addMember = async (e) => {
    ...
    try {
      await api.post(`/homecells/${id}/members/`, { user_id: Number(addUserId) })
      setAddUserId('')
      setNotice('Member registered to this Home Cell.')
      load()
    } catch (err) {
  ...
  const removeMember = async (userId) => {
    if (!confirm('Remove this member from the Home Cell?')) return
    setError('')
    setNotice('')
    try {
      await api.delete(`/homecells/${id}/members/${userId}/`)
    ...
    <div className="space-y-8">
      <div className="rounded-3xl bg-white p-8 shadow-sm">
        ...
        <p className="text-slate-600 mt-2">
          <Link to={`/homecells/${id}/dashboard`} className="text-primary-600 font-semibold hover:underline">Open this Home Cell's console →</Link>
        </p>
      </div>
      ...
      <form onSubmit={saveForm} className="rounded-3xl bg-white p-8 shadow-sm border border-slate-100 space-y-4">
        <h2 className="text-xl font-semibold">Home Cell Information</h2>
      ...
      <div className="rounded-3xl bg-white p-8 shadow-sm border border-slate-100 space-y-4">
        ...
        {assignments.length >= MAX_DEPARTMENT_ADMINS ? (
          <p className="text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3">
            This Home Cell already has the maximum of {MAX_DEPARTMENT_ADMINS} admins. Remove one before assigning another.
          </p>
        ) : (
          <form onSubmit={assignLeader} className="flex flex-wrap items-end gap-3">
            ...
            <label className="w-full">
              ...
              <textarea
                ...
                placeholder="e.g. Oversees weekly meetings and attendance tracking"
                className="mt-2 w-full rounded-2xl border border-slate-300 px-4 py-3"
              />
            </label>
          </form>
        )}
        <p className="text-xs text-slate-500">Assigning a member here grants them access to this Home Cell's console, promoting them to Administrator role if needed.</p>
      ...
      <div className="rounded-3xl bg-white p-8 shadow-sm border border-slate-100 space-y-4">
        ...
        <form onSubmit={addMember} className="flex flex-wrap items-end gap-3">
          <label className="flex-1 min-w-[200px]">
            <span className="text-slate-700 text-sm">Register or transfer a member</span>
          ...
          <button disabled={addingMember || !addUserId} type="submit" className="rounded-2xl bg-slate-900 text-white px-6 py-3 font-semibold disabled:opacity-60">
            {addingMember ? 'Adding…' : 'Add / Transfer In'}
          </button>
        </form>
        <p className="text-xs text-slate-500">Each member belongs to one Home Cell — adding them here transfers them off any cell they currently belong to.</p>
